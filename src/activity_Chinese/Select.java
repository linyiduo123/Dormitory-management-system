...
//        Connection();
        QUERY = new JFXButton("综合查询");
        CHECK_IN = new JFXButton("办理入住");
        CHECK_OUT = new JFXButton("办理退房");
        ...
        CHECK_OUT.setStyle("-jfx-button-type: RAISED;" +
                " -fx-background-color: white;" +
                "-fx-font-size:15;");
        Label choosen = new Label("选择的寝室是：");
        ...
        temp2.setStyle("-jfx-button-type: RAISED;" +
                ...
                "-fx-text-fill: white;");
        Label ym = new Label("已满");
        Label wm = new Label("未满");
...
//        }
        ...
        CHECK_IN.setOnAction(e->
        {
            if (choosen_num.getText().length()==3){
                CheckIn checkIn = new CheckIn(choosen_num.getText());
                try {
                    checkIn.start(stage);
                ...
                Alert error = new Alert(Alert.AlertType.WARNING,"请选择房间！");
                error.showAndWait();
            }
        });
        ...
        {
            if (choosen_num.getText().length()==3){
                CheckOut checkIn = new CheckOut(choosen_num.getText());
                try {
                    checkIn.start(stage);
                } catch (SQLException e1) {
                    e1.printStackTrace();
