...
public class CheckOut extends Application {
    ...
    public void start(Stage stage) throws SQLException, IOException {
        ...
        SUBMIT.setOnAction(e->{
            ...
            if (ID.length()==0) {
                Alert error = new Alert(Alert.AlertType.WARNING,"请输入学号！");
                error.showAndWait();
            }
            else if (NAME.length()==0) {
                Alert error = new Alert(Alert.AlertType.WARNING,"请输入姓名！");
                error.showAndWait();
            }
            else if (bed.getText().length()==0) {
                Alert error = new Alert(Alert.AlertType.WARNING,"请选择要入住的床位！");
                error.showAndWait();
            }
            ...
            try{
                 ...
                 * 各条件串行判断
                 *
                 * 判断顺序： 床位是否无人 > 姓名学号是否匹配 > 学号是否是该床位入住学生
                 *                     > 姓名是否与该学号匹配 > 该学生是否属于该寝室
                 *
                 */
                if (rs2.next()){
                    if (rs2.getString(BED).length()==1){
                        Alert error = new Alert(Alert.AlertType.WARNING,"该床位暂时无人入住，请重新选择！");
                        error.showAndWait();
                        clearInput();
                    }
                    else if (!rs2.getString(BED).equals(ID)){
                        Alert error = new Alert(Alert.AlertType.WARNING,"身份信息匹配错误！请重试");
                        error.showAndWait();
                        clearInput();
                    }
                    else {
                        ...
                        if (rs.next()) {
                            if (!rs.getString("stuName").equals(NAME)){
                                ...
                                if (rs.getString("isCheckIn").length()==3){
                                    ...
                                    WriteLog(log_string);
                                    checkBed();
                                    Alert error = new Alert(Alert.AlertType.INFORMATION,"退房成功！");
                                    error.showAndWait();
                                    clearInput();
                                }
                                else {
                                    Alert error = new Alert(Alert.AlertType.WARNING,"身份信息验证错误！");
                                    error.showAndWait();
                                    clearInput();
                                }
                            }
