...
public class CheckIn_E extends Application {
    String id;
    Statement sta = null;
    Connection con = null;
    ResultSet rs = null;
            ...
            try{
                ...
                if (rs2.next()){
                    if (rs2.getString(BED).length()==7){
                        Alert error = new Alert(Alert.AlertType.WARNING,"This bed has been choosen, try again!");
                        error.showAndWait();
                        clearInput();
                    }
                    else {
                        ...
                        if (rs.next()) {
                            if (!rs.getString("stuName").equals(NAME)){
                                ...
                                if (rs.getString("isCheckIn").length()!=3){
                                    ...
                                    WriteLog(log_string);
                                    Alert error = new Alert(Alert.AlertType.INFORMATION,"Check in success！");
                                    error.showAndWait();
                                    checkBed();
                                    clearInput();
                                }
                                else {
                                    Alert error = new Alert(Alert.AlertType.WARNING,"You had checked in a room！\nPlease check out before check in a new room！");
                                    error.showAndWait();
                                    clearInput();
                                }
                            }
